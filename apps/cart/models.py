...
class Cart(object):
    ...
    def add(self, product, quantity, action=None):
        ...
        self.cart[product.barcode][
            "tax_value"
        ] = f"{product.sales_price * self.cart[product.barcode]['quantity']* (product.tax_category.tax_percentage/100):.2f}"
        self.cart[product.barcode][
            "deposit_value"
        ] = f"{self.cart[product.barcode]['quantity']* product.deposit_category.deposit_value:.2f}"
        self.cart[product.barcode][
            "line_total"
        ] = f"{(product.sales_price * self.cart[product.barcode]['quantity']) + Decimal(self.cart[product.barcode]['tax_value']) + Decimal(self.cart[product.barcode]['deposit_value']):.2f}"
        self.save()
    ...
    def returns(self):
        for key, value in self.cart.items():
            value["quantity"] = value["quantity"] * (-1)
            value["tax_value"] = (
                float(value["tax_value"])
                if float(value["tax_value"]) == 0.0
                else float(value["tax_value"]) * (-1)
            )
            value["deposit_value"] = (
                float(value["deposit_value"])
                if float(value["deposit_value"]) == 0.0
                else float(value["deposit_value"]) * (-1)
            )
