...
class TransactionAdmin(ImportExportModelAdmin):
    ...
    list_filter = (
        ("transaction_dt", DateTimeRangeFilter),
        "transaction_dt",
        "user",
        "payment_type",
    )
    ...
    def receipt_link(self, obj=None):
        if obj is not None:
            return format_html(
                f'<a href="/transaction_receipt/{obj}" style="color:green;" target="_blank">View Receipt</a>'
            )
    ...
    def products_link(self, obj):
        ...
        return format_html(
            '<a href="{}" style="color:#4e73df;">{} Product Transaction</a>', url, count
        )
...
class ProductTransactionAdmin(ImportExportModelAdmin):
    ...
    list_filter = [
        ("transaction_date_time", DateTimeRangeFilter),
        "department",
        "tax_category",
        "deposit_category",
        "payment_type",
    ...
    def link_transaction(self, obj=None):
        if obj is not None:
            return format_html(
                f'<a href="/staff_portal/transaction/transaction/{obj.transaction.id}/change/" style="color:#4e73df;">{obj.transaction}</a>'
            )
