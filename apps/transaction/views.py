...
class printer:
    ...
    def connectPrinter():
        try:
            printer.printer = Usb(
                eval(settings.PRINTER_VENDOR_ID), eval(settings.PRINTER_PRODUCT_ID)
            )
        except Exception as e:
            print(e)
...
def transactionReceipt(request, transNo):
    try:
        ...
        return render(
            request,
            "receiptView.html",
            context={"receipt": receipt, "transNo": transNo},
        )
...
def transactionView(request, transNo=None):
    ...
    transactions = (
        transaction.objects.filter(transaction_dt__date__range=(start_date, end_date))
        .order_by("-transaction_dt")
        .values("transaction_dt", "transaction_id", "total_sale", "payment_type")
    )
    return render(
        request,
        "transactions.html",
...
def recallTransaction(request, recallTransNo=None):
    ...
    if recallTransNo:
        request.session[settings.CART_SESSION_ID] = request.session["Cart_Sessions"][
            recallTransNo
        ]
        del request.session["Cart_Sessions"][recallTransNo]
        request.session.modified = True
    elif "Cart_Sessions" in request.session.keys() and len(
        request.session["Cart_Sessions"]
    ):
        return render(
            request,
            "recallTransaction.html",
            context={"obj_rt": request.session["Cart_Sessions"].keys()},
        )
    return redirect("register")
...
def endTransactionReceipt(request, transNo):
    try:
        ...
        return render(
            request,
            "endTransaction.html",
            context={"receipt": obj.receipt, "change": change},
        )
    except Transaction.DoesNotExist:
        raise Http404("No Transactions Found!!!")
        ...
        if type == "card":  # Card Transaction
            ...
            if value == "EBT":
                return_transaction = addTransaction(
                    request.user, "EBT", total, cart, total
                )
            # DEBIT/CREDIT Transaction
            elif value == "DEBIT_CREDIT":
                return_transaction = addTransaction(
                    request.user, "DEBIT/CREDIT", total, cart, total
                )
        elif type == "cash":  # Cash Transaction
            value = round(float(value), 2)
            if value >= total:
                return_transaction = addTransaction(
                    request.user, "CASH", total, cart, value
                )
        if return_transaction:
            Cart(request).clear()
            return redirect(
                f"/endTransaction/{return_transaction.transaction_id}/?type={type}&value={value}&total={total}"
            )
        return redirect("register")
    except Exception as e:
...
def addTransaction(user, payment_type, total, cart, value):
    ...
    cart_df["tax"] = (
        cart_df["tax_value"]
        .astype(float)
        .apply(lambda x: "T" if x > 0 else "-T" if x < 0 else "")
    )
    cart_df["deposit"] = (
        cart_df["deposit_value"].astype(float).apply(lambda x: "" if x == 0.00 else x)
    )
    ...
    cart_string = "\n".join(
        list(
            cart_df.apply(
                lambda row: f"{str(row.name)+')':<3} {row['name'][:28]}".ljust(
                    settings.RECEIPT_CHAR_COUNT
                )
                + "\n"
                + f" {row['barcode']:<13}{row['quantity']:>3}{row['price']:>7}{row['deposit']:>6}{row['tax']:>2}".rjust(
                    settings.RECEIPT_CHAR_COUNT
                ),
                axis=1,
            )
    ...
    total_string = f"Sub-Total: {round(total-tax_total,2)}  Tax-Total: {round(tax_total,2)}".center(
        settings.RECEIPT_CHAR_COUNT
    )
    total_string = (
        total_string
    ...
    total_string = (
        total_string
        + "\n"
        + f"{str(payment_type):>10}: $ {round(value,2):.2f}".rjust(
            settings.RECEIPT_CHAR_COUNT
        )
    )
    total_string = (
        total_string
        + "\n"
        + f"{'CHANGE':>10}: $ {round(value-total,2):.2f}".rjust(
            settings.RECEIPT_CHAR_COUNT
        )
    )
    ...
    receipt = "\n".join(
        [i.center(settings.RECEIPT_CHAR_COUNT) for i in receipt.splitlines()]
    )
