...
class AddProduct(forms.Form):
    qty = forms.IntegerField(
        label="Quantity To Be Added", widget=TextInput(attrs={"style": "width:100%"})
    )
    barcode = forms.CharField(
...
def product_lookup(request):
    ...
    if obj:
        return render(
            request,
            "productLookup.html",
            context={"form": form, "notFound": notFound, "obj": obj},
        )
    return render(
        request, "productLookup.html", context={"form": form, "notFound": notFound}
    )
