...
from apps.cart.models import Cart, DisplayItem
from apps.inventory.models import Product
from apps.transaction.models import ProductTransaction, Transaction
from apps.transaction.views import DateSelector
...
class EnterBarcode(forms.Form):
    ...
    qty = forms.IntegerField(
        label="Quantity", widget=forms.TextInput(attrs={"style": "width:100%"})
    )
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            return redirect(
                f"/cart/add/{form.cleaned_data['barcode']}/{form.cleaned_data['qty']}"
            )
    try:
    ...
    context = {
        ...
        "total": Total,
        "tax_total": Tax_Total,
        "displayed_items": DisplayItem.objects.all(),
    }
...
def retail_display(request, values=None):
    ...
    if os.path.exists(f"./{path}"):
        ...
        shutil.copytree(
            f"./{path}", f"{settings.STATIC_ROOT}/{path}", dirs_exist_ok=True
        )
    ...
    return render(
        request,
        "retailDisplay.html",
        context={"store_name": settings.STORE_NAME, "display_images": img_list},
    )
...
def report_regular(request, start_date, end_date):
    ...
    df["transaction_date_time"] = df["transaction_date_time"].apply(
        lambda x: x.astimezone(timezone)
    )
    df["date"] = df["transaction_date_time"].dt.date
    df["total_sales"] = (
        (df["qty"] * df["sales_price"]) + df["tax_amount"] + df["deposit_amount"]
    )
    df["total_pre_sales"] = df["qty"] * df["sales_price"]

    date_group = df.groupby(["date", "department", "payment_type"])[
        ["qty", "total_pre_sales", "tax_amount", "deposit_amount", "total_sales"]
    ].apply(lambda x: x.sum())
    table = (
        date_group.reset_index()
        .groupby(["date"])[
            ["total_pre_sales", "tax_amount", "deposit_amount", "total_sales"]
        ]
        .apply(lambda x: x.sum())
    )
    ...
    date_group.loc[("TOTAL", "TOTAL", " TOTAL")] = df[
        ["total_pre_sales", "tax_amount", "deposit_amount", "total_sales"]
    ].apply(lambda x: x.sum())
    for i, val in (
...
def dashboard_products(request):
    try:
        ...
        df = pd.DataFrame(
            ProductTransaction.objects.filter(
                transaction_date_time__date__range=(last_30_date, today_date)
            )
            .order_by("-transaction_date_time")
            .values()
        )
        ...
        for i, df in df.groupby("department"):
            context["products_group"][i] = (
                df.groupby(["barcode", "name"])[["qty"]]
                .sum()
                .reset_index()
        ...
        context["low_inventory_products"] = (
            Product.objects.all().order_by("qty").values("barcode", "name", "qty")[:50]
        )
...
def dashboard_department(request):
    ...
    if df.shape[0]:
        ...
        tableValues = [
            ["Total QTY", "Total Sales b4 Tax & Deposit", "Total Tax", "Total Deposit"]
        ...
        tableValues = [("TOTAL SALES", round(df["total_sales"].sum(), 2))] + list(
            zip(tableValues[0], tableValues[1])
        )
        ...
        sales_by_department = df.groupby(["department", "payment_type"])[
            ["qty", "total_pre_sales", "tax_amount", "deposit_amount", "total_sales"]
        ].apply(lambda x: x.sum())
        ...
        bar_fig = px.bar(
            ...
            labels={
                ...
                "total_sales": "Total Sales",
                "total_pre_sales": "Total Sales b4 Tax & Deposit",
        ...
        bar_fig.update_yaxes(
            title=f"Total Sales ({start_date:%Y/%m/%d} - {end_date:%Y/%m/%d})"
        )
        bar_fig.update_layout(
            margin=dict(b=10, pad=0, t=10, l=10, r=10), height=500, showlegend=False
        )
    ...
    try:
        df = pd.DataFrame(
            Transaction.objects.filter(
                transaction_dt__date__gte=datetime(today_date.year, 1, 1)
            ).values()
        )
        df["transaction_dt"] = df["transaction_dt"].apply(
            lambda x: x.astimezone(timezone)
        )
        df["date"] = df["transaction_dt"].dt.date
        df_date = df.groupby("date")["total_sale"].sum()
        df_date.index = pd.to_datetime(df_date.index)
        if not df_date.get(datetime(today_date.year, 1, 1)):
            df_date[datetime(today_date.year, 1, 1)] = 0
        if not df_date.get(today_date):
            df_date[today_date] = 0
        df_date = df_date.asfreq("D", fill_value=0)

        context["today_total_sales"] = df_date.get(today_date)
        context["add_info"] = {}
        context["add_info"]["Yesterday's Total Sales"] = df_date.get(
            today_date - timedelta(1)
        )
        context["add_info"]["Last 7 Days Avg Sales"] = (
            df_date[df_date.index > today_date - timedelta(7)].sum() / 7
        )
        context["30_Days_Avg_Sales"] = df_date[
            df_date.index > today_date - timedelta(30)
        ].mean()
        context["30_Days_Total_Sales"] = df_date[
            df_date.index > today_date - timedelta(30)
        ].sum()
        ...
        fig.update_layout(
            margin=dict(b=10, pad=0, t=10, r=0, l=0),
        )
        div = po.plot(
            fig,
            auto_open=False,
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(
                ...
                context={"error": True, "store_name": settings.STORE_NAME},
            )
    else:
        return render(
            request,
